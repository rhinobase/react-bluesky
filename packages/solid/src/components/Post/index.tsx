...
export function Post({ thread: { post, parent, replies } }: Post) {
  ...
  return (
    <Container href={href}>
      <div class={s.post} lang={record?.langs?.[0]} dir="auto">
...
function Header({ author, href }: Header) {
  ...
  return (
    <div class={s.header}>
      <Link href={`/profile/${author.did}`} class={s.avatarLink}>
        <div class={s.avatar}>
          <img
            alt={author.displayName}
            src={author.avatar}
            class={classNames(isAuthorLabeled && s.avatarImg)}
          />
        </div>
      </Link>
      <div>
        <Link href={`/profile/${author.did}`} class={s.displayName}>
          <p>{author.displayName}</p>
        </Link>
        <Link href={`/profile/${author.did}`} class={s.handle}>
          <p>@{author.handle}</p>
        </Link>
      </div>
      <div class={s.spacer} />
      <Link href={href} class={s.logoLink}>
        <svg
...
function CreatedAt({ indexedAt, href }: CreatedAt) {
  return (
    <Link href={href}>
      <time dateTime={new Date(indexedAt).toISOString()} class={s.createdAt}>
        {niceDate(indexedAt)}
      </time>
    </Link>
  );
    ...
    <div class={s.actions}>
      {!!likeCount && (
        <div class={s.action}>
          ...
          <p class={s.actionText}>{prettyNumber(likeCount)}</p>
        </div>
      )}
      {!!repostCount && (
        <div class={s.action}>
          ...
          <p class={s.actionText}>{prettyNumber(repostCount)}</p>
        </div>
      )}
      <div class={s.action}>
        <svg
        ...
        <p class={s.actionText}>Reply</p>
      </div>
      <div class={s.spacer} />
      <p class={classNames(s.replies, s.repliesCount)}>
        {replyCount
      ...
      <p class={classNames(s.replies, s.viewOnBluesky)}>
        <span class={s.viewOnBlueskyText}>View on </span>Bluesky
      </p>
    </div>
  );
}
